...
with open('src/views/DashboardOneHealth.vue', 'r', encoding='utf-8') as f:
    text = f.read()
...
new_template = """<template>
  <section class="relative min-h-screen overflow-hidden pb-20">
    ...
    <transition name="modal">
      <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 fade-up">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-black/60 backdrop-blur-md" @click="cancelEdit"></div>
        
        <div class="relative w-full max-w-3xl max-h-[90vh] overflow-y-auto custom-scroll bg-[#0a160c] rounded-3xl shadow-2xl border border-white/10 p-7 xl:p-10 z-10">
          <button @click="cancelEdit" class="absolute top-6 right-6 p-2 rounded-full bg-black/40 text-white/50 hover:text-white hover:bg-black/60 transition-colors border border-white/10 z-20">
...
final_text = re.sub(r'<template>.*?</template>', new_template, text, flags=re.DOTALL)

with open('src/views/DashboardOneHealth.vue', 'w', encoding='utf-8') as f:
    f.write(final_text)
